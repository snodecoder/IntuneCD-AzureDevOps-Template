...
#### Policy Backup
This screenshot shows an example .json backup of an application after the successful execution of the `intune-backup.yml` pipeline in Azure DevOps.
![Policy Backup](.img/Azure-DevOps-Policy-Backup.png)
...
<img width="746" height="857" alt="image" src="https://github.com/user-attachments/assets/2d3c6724-169e-4d73-b400-ada1a3b531bc" />

![Generated Documentation](.img/Azure-DevOps-Wiki.png)

#### Audit Commit Message
This screenshot the added commit messages after running the backup pipeline. Every changed policy is linked to an Intune audit log event (of the past 24 hours). The commit message is added with the username of the Admin that made the change in Intune. The commmit itself contains the changed files.

All changed policies that couldn't be linked to an Intune audit log event (in past 24 hours) are committed together in the `Intune Config Backup [date]` commit.

Documentation changes are committed in the `Intune config as-built [date]` commit, together with a tag that contains the date of the backup.
![Audit Commit Message](.img/Azure-DevOps-Audit-Commit-Message.png)
